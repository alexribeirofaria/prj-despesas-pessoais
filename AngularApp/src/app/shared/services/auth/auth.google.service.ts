...
export class AuthGoogleService {
  ...
  handleGoogleLogin(): Observable<IAuth> {
            ...
            this.handleCredentialResponse(response)
              .pipe(
                map((res) => {
                  ...
                  throw new Error(res.message || 'Autenticação falhou');
                }),
  ...
  private handleCredentialResponse(response: any): Observable<IAuth> {
    ...
    return this.acessoService.signInWithGoogleAccount(authData).pipe(
      ...
      catchError((err) => throwError(() => new Error(err?.message || err)))
    );
  }
  ...
  logout(): void {
    ...
    this.authProviderService.clearSessionStorage();
