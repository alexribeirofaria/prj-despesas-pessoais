...
public sealed class ReceitaParserTest
{
    ...
    {
        // Arrange
        var options = new DbContextOptionsBuilder<RegisterContext>().UseInMemoryDatabase(databaseName: "ReceitaParserTest").Options;

        using (var context = new RegisterContext(options, DatabaseProvider.MySql, Usings.GetLogerFactory()))
        {
            var builder = new ModelBuilder(new ConventionSet());
            var configuration = new ReceitaMap(DatabaseProvider.MySql);
